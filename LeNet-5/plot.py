...
leggy=ax.legend(loc=0,framealpha=5,  fontsize='x-large') #------Legend place
...
for k in range(avg_iterations):
	avg_mi[k,:,:]= np.mean(MI[k*100 : (k+1)*100],axis=0)
...
leggy=ax.legend(loc='lower right',framealpha=5, fontsize='x-large') #------Legend place
leggy.get_frame().set_edgecolor('black') #-----Legend frame color
...
plt.gcf().savefig('plots/'+file_name + "mi_after_prune.png")
plt.show()
...
MI=MI[:initial_iter,:,:]
avg_iterations=MI[:initial_iter,:,:].shape[0]//100
