...
class Network():
    ...
    def cal_mi(self, x,k_x,k_y , model,layer, current_iteration):
      ...
      k_layer=self.kernel_mat(val.reshape(data[0].size(0), -1),k_y, epoch=current_iteration,idx=layer)
      k_list=[]
      k_list.append(k_x)
    ...
    def remaining_filters_per_epoch(self,model=None,initial=None):
        #print(model,initial)
        blocks=["layer1.0.conv1","layer2.0.conv1","layer3.0.conv1"]
        if(initial == True):
           layer_number=0 
           for layer_name, layer_module in model.named_modules():
              #if(isinstance(layer_module, th.nn.Conv2d) or  isinstance(layer_module,th.nn.Linear)):
              if(isinstance(layer_module, th.nn.Conv2d)  and layer_name in blocks):
                 self.remaining_filters_each_epoch.append([])
                 print(layer_name)
        for i in range(len(blocks)):
            self.remaining_filters_each_epoch[i].append(len(self.remaining_filters[18*i])) #--18 convs in each block
