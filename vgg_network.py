...
class Network():
    ...
    def train_model(self, x, y, model,optimizer,scheduler, gpu):

        '''for name, param in model.named_parameters():
           if param.requires_grad:
              print(name, param.size(),type(param.data),(param.data).dtype)'''
        #below line modified
        #optimizer = th.optim.SGD(model.parameters(), lr=0.01,momentum=0.9,weight_decay=5e-4)
        #scheduler = optimizer.lr_scheduler.MultiStepLR(optimizer, milestones=[5,10], gamma=0.1)
    ...
    def compute_bounds(self,x, y, model, gpu,batch_num, current_iteration,layers_for_pruning,folder_name,decision_count,no_of_batches):
      ...
      for layer in range(layers_for_pruning):
          ...
          #sigma_calculated= self.sigmas[layer,current_iteration]
          #sigma_calculated= self.sigmas[layer,-1] sigmas_fixed
          #print('sigma ',self.sigmas_fixed[layer])
      ...
      if(batch_num+1 == no_of_batches):
            ...
            for i in range(layers_for_pruning):#-------------------2 conv layers
              ...
              #z.append(((th.sum(self.sub_MI_X[i],dim=0)-(beta *  th.sum(self.sub_MI_Y[i],dim=0))).div(no_of_batches)).tolist())
              ...
              #layers_x=[]
              layers_y=[]
              #layers_x.extend(float("{:.6f}".format(x.item())) for x in th.sum(self.sub_MI_X[i],dim=0))
              #layers_y.extend(float("{:.6f}".format(x.item())) for x in th.sum(self.sub_MI_Y[i],dim=0))
              
              l1norm_rem=[]
              for layer_name, layer_module in model.named_modules():
                 
                 if(layer_name == self.layer_name_num[i]):
                    filter_weight=layer_module.weight.clone()
                    
                    for k in range(filter_weight.size()[0]):
                      if(k not in self.remaining_filters[i]):
                        l1norm_rem.append(float("{:.6f}".format(th.norm(filter_weight[k,:,:,:]).item())))
              print('rem layers of ',i,'=',sum(l1norm_rem))
              z[-1]=[float("{:.7f}".format(i)) for i in z[-1]]
              small_z[-1]=[float("{:.7f}".format(i)) for i in small_z[-1]]
              #print('z[-1]',len(z[-1]),'z',len(small_z[-1]))
              d=[self.remaining_filters[i],z[-1],small_z[-1]]
              export_data = zip_longest(*d, fillvalue = '')
              #with open('/home/hareesh/Documents/sar/information_theory/Results/LeNetMnistPrune/conv'+str(i)+'.csv', 'a', newline='') as myfile:
              
              with open(folder_name+'layer_file_info/conv'+str(i)+'.csv', 'a', newline='') as myfile:
                  wr = csv.writer(myfile)
                  wr.writerow(("filter","I(F:Y)","small_IY"))
            ...
            for i in range(layers_for_pruning):
              ...
              #plt.savefig("/home/hareesh/Documents/sar/information_theory/IBbound_distributions/LeNet_MNIST/conv"+
